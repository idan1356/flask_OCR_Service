...
def add_sentence_outline_to_image(image_bytes: bytes, reader_output: List, file_extension: str) -> bytes:
    image = Image.open(io.BytesIO(image_bytes))
    draw = ImageDraw.Draw(image)

    for entry in reader_output:
        coordinates = [tuple(point) for point in entry]
        draw.polygon(coordinates, outline=(0, 255, 0), width=2)

    output_image_io = io.BytesIO()
    image.save(output_image_io, format=file_extension)
